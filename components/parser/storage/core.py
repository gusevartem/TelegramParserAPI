    ...
    async def save_media(
        ...
        for attempt in range(max_retries):
            try:
                ...
                wait = 2**attempt
                self.logger.warning(
                    f"Error while trying to upload file ({type(e).__name__}). "
                    + f"Retry {attempt + 1}/{max_retries} after {wait}s. "
                    + f"Error: {e}"
                )
                await asyncio.sleep(wait)
    ...
    async def generate_presigned_url(
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.info(f"Generated presigned url for media {file_name}")
                return url
                ...
                if code == "AccessDenied":
                    ...
                    wait = 2**attempt
                    self.logger.warning(
                        f"S3 Error {code} during URL gen. "
                        + f"Retry {attempt + 1}/{max_retries} after {wait}s"
                    )
                    await asyncio.sleep(wait)
                ...
                wait = 2**attempt
                self.logger.warning(
                    f"Error generating URL ({type(e).__name__}). "
                    + f"Retry {attempt + 1}/{max_retries} after {wait}s. "
                    + f"Error: {e}"
                )
                await asyncio.sleep(wait)
